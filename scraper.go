...
func (cfg *apiConfig) fetchFeed(feed Feed) (RssFeed, error){
	...
	log.Println("read feed:", data.Channel.Title, data.Channel.Link)
	return data, nil

}

func (cfg *apiConfig) processFeed(feed Feed, rss RssFeed) {
	for _, item := range rss.Channel.Items {
		log.Printf("[%s] Fetching post: %s", rss.Channel.Title, item.Title)
